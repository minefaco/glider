...
local function get_pitch_lift(y)
	return -(1964/1755*y^4)-(2549/3510*y^2)+(2591/7020*y^2)+(2594/3510*y)+0.75
end
...
local function on_step(self, dtime, moveresult)
	...
	if moveresult and moveresult.collisions and moveresult.collides then
		for _, collision in pairs(moveresult.collisions) do
			land = true
			crash_speed = crash_speed + math.abs(collision.old_velocity.x - collision.new_velocity.x) +
	...
	if land then
		...
		if crash_damage > 0 then
			...
			if minetest.registered_nodes[node.name].liquidtype == "none" then
				driver:set_hp(driver:get_hp() - crash_damage, {type = "fall"})
			end
		end
		self.object:remove()
		return
	...
	if mouse_controls then
		...
		if control.up then rot.x = rot.x + dtime end
		if control.down then rot.x = rot.x - dtime end
		if control.left then rot.z = rot.z - 2 * dtime end
		if control.right then rot.z = rot.z + 2 * dtime end

		if rot.z ~= 0 then
			speed = speed - math.abs(rot.z * dtime)
			if math.abs(rot.z) < 0.01 then rot.z = 0 end
